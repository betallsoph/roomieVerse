...
export default function AuthForm({ bounceKey = 0, onModeChange }: AuthFormProps) {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      if (firebaseError.code === 'auth/popup-blocked') {
        setError("Popup bị chặn bởi trình duyệt. Vui lòng cho phép popup và thử lại.");
      } else if (userCancelled.includes(firebaseError.code || '')) {
        setError("Đã hủy đăng nhập. Nhấn để thử lại.");
      } else {
  ...
  async function handleForgotPassword(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccessMsg("Link đặt lại mật khẩu đã được gửi đến email của bạn.");
    } catch (err: unknown) {
      const firebaseError = err as { code?: string };
      const errorMap: Record<string, string> = {
        "auth/user-not-found": "Không tìm thấy tài khoản với email này.",
  ...
  return (
    ...
    >
      ...
      {mode === "forgot" ? (
        <div className="mt-8">
          <h2 className="text-xl font-bold text-black mb-2">Quên mật khẩu?</h2>
          <p className="text-sm text-zinc-500 mb-6">Nhập email để nhận link đặt lại mật khẩu.</p>

          <form autoComplete="off" className="space-y-5" onSubmit={handleForgotPassword}>
            ...
            {successMsg && (
              <div className="rounded-xl border-2 border-green-400 bg-green-50 px-4 py-3 text-sm font-medium text-green-700">
                {successMsg}
              </div>
            )}
          ...
          <button
            type="button"
            onClick={() => handleModeChange("login")}
            className="mt-4 w-full text-center text-sm font-semibold text-blue-500 hover:text-blue-700 transition-colors"
          >
            Quay lại đăng nhập
          </button>
        </div>
          ...
          <div className="mt-8">
            ...
            {isGoogleLoading && (
              <button
                ...
                className="mt-2 w-full text-center text-sm font-medium text-zinc-500 hover:text-zinc-700 transition-colors"
              >
                Hủy
              </button>
            )}
