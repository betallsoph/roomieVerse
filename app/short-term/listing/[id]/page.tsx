...
export default function ShortTermListingDetailPage() {
  useAdminRedirect();
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="flex min-h-[60vh] items-center justify-center">
          <div className="rounded-xl border-2 border-black bg-white p-16 text-center shadow-[var(--shadow-primary)]">
            ...
            <Link href="/short-term" className="inline-flex items-center gap-2 rounded-lg border-2 border-black bg-yellow-300 px-6 py-3 font-bold shadow-[2px_2px_0_0_#000]">
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="border-b-2 border-black bg-yellow-50 py-12 sm:py-16">
        <div className="mx-auto max-w-7xl px-6">
          {/* Breadcrumb */}
          <div className="mb-6 flex items-center gap-2 text-sm text-zinc-600">
            <Link href="/" className="hover:text-black">Trang chủ</Link>
            <span>/</span>
            <Link href="/short-term" className="hover:text-black">Phòng ngắn ngày</Link>
            <span>/</span>
      ...
      <div className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
          ...
          <div className="space-y-8">
                      ...
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                        {images.map((_, idx) => (
                          <button
                            ...
                            className={`w-3 h-3 rounded-full border-2 border-black ${idx === currentImageIndex ? "bg-yellow-400" : "bg-white"}`}
                          />
                        ))}
                      </div>
                    </>
            ...
            <div className="rounded-xl border-2 border-black bg-white p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-4 text-sm">
                <div>
                  <p className="text-zinc-500 mb-1">Giá cho thuê</p>
                  <p className="text-xl font-bold text-amber-600">{listing.price}</p>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-zinc-100">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {displayAmenities.map((amenity) => (
                      <span
                        key={amenity}
                        className="px-3 py-1 bg-yellow-50 text-amber-600 rounded-full text-sm font-medium"
                      >
                        {amenity}
                      </span>
                    ))}
            ...
            {/* Description Card */}
            <div className="rounded-xl border-2 border-black bg-white p-6">
              <h3 className="text-lg font-bold mb-3 text-amber-600">Mô tả</h3>
            ...
            <div className="rounded-xl border-2 border-black bg-red-50 p-5 shadow-[var(--shadow-secondary)]">
              <h2 className="mb-3 text-lg font-bold">Lưu ý khi thuê ngắn ngày</h2>
              <ul className="space-y-1.5 text-sm text-zinc-700">
                <li>• Xem phòng trực tiếp trước khi quyết định</li>
                <li>• Thống nhất rõ thời gian check-in/check-out</li>
                <li>• Hỏi rõ các chi phí phát sinh (điện, nước, dọn vệ sinh)</li>
          ...
          <div className="lg:sticky lg:top-24 space-y-4">
            {/* Contact Card */}
            <div className="rounded-xl border-2 border-black bg-yellow-50 p-6 shadow-[var(--shadow-secondary)]">
              {listing.userId ? (
                <Link href={`/user/${listing.userId}`} className="mb-5 flex items-center gap-3 group">
                  <div className="w-12 h-12 rounded-full bg-yellow-100 flex items-center justify-center border-2 border-black group-hover:border-amber-500 transition-colors">
                    <span className="text-lg font-bold text-amber-600">{listing.author?.charAt(0)?.toUpperCase() || "?"}</span>
                  </div>
                  <div>
                    <p className="text-xl font-bold group-hover:text-amber-600 transition-colors">{listing.author}</p>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 rounded-lg border-2 border-black px-3 py-2.5 text-sm font-bold transition-all hover:translate-x-[2px] hover:translate-y-[2px] shadow-[var(--shadow-secondary)] hover:shadow-none
                    ${isFavorited ? "bg-yellow-300" : "bg-white"}`}
                >
              ...
              <div className="pt-4 border-t border-zinc-200">
                ...
                <div className="space-y-3">
                  {similarListings.map((item) => (
                    <Link
                      key={item.id}
                      href={`/short-term/listing/${item.id}`}
                      className="block p-3 rounded-lg border-2 border-black bg-white hover:bg-yellow-50 transition-colors"
                    >
