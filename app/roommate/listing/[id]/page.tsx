...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  const description = (listing.introduction || listing.description || "").slice(0, 160);
  const address = [listing.district, listing.city].filter(Boolean).join(", ");

  return {
    title,
    description: description || `Tìm Roommate tại ${address}`,
    openGraph: {
      title,
      description,
      type: "website",
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              name: listing.title,
              description: listing.introduction || listing.description,
              url: `https://roomieverse.app/roommate/listing/${id}`,
              ...(listing.price && {
                offers: {
                  "@type": "Offer",
                  price: listing.price,
