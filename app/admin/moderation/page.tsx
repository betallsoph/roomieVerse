...
import { getPendingListings, getRejectedListings, approveListing, rejectListing, hardDeleteListing } from "../../data/listings";
import { RoomListing } from "../../data/types";
import {
  ArrowLeft, Shield, CheckCircle, XCircle, Eye, Clock, MapPin, Calendar, Loader2,
  Home, Users, Building2, UserSearch, Ban, RotateCcw, Trash2
} from "lucide-react";

// Listing sub-filter
type ListingFilter = "all" | "have-room" | "find-partner" | "roomshare-house" | "roomshare-apartment" | "short-term" | "sublease" | "rejected";

const listingFilters: { key: ListingFilter; label: string; icon: React.ElementType | null }[] = [
  ...
  { key: "rejected", label: "Đã từ chối", icon: Ban },
];
...
export default function ModerationPage() {
  ...
  const [rejectedPosts, setRejectedPosts] = useState<RoomListing[]>([]);
  ...
  // Fetch pending + rejected listings
  useEffect(() => {
  ...
  // Filtered posts
  const isRejectedView = listingFilter === "rejected";
  const filteredPosts = useMemo(
    () => isRejectedView ? rejectedPosts : pendingPosts.filter(p => matchesFilter(p, listingFilter)),
    [pendingPosts, rejectedPosts, listingFilter, isRejectedView]
  );
  ...
  const handleReject = async (listingId: string) => {
    ...
    setActionLoading(listingId);
    try {
      const rejectedPost = pendingPosts.find(p => String(p.id) === listingId);
      await rejectListing(listingId, user.uid, rejectReason.trim());
      setPendingPosts(prev => prev.filter(p => String(p.id) !== listingId));
      if (rejectedPost) {
        setRejectedPosts(prev => [{ ...rejectedPost, status: "rejected" as const, rejectionReason: rejectReason.trim() }, ...prev]);
      }
      setRejectingId(null);
      setRejectReason("");
    } catch (error) {
  ...
  const handleDelete = async (listingId: string) => {
    ...
    try {
      ...
      setPendingPosts(prev => prev.filter(p => String(p.id) !== listingId));
      setRejectedPosts(prev => prev.filter(p => String(p.id) !== listingId));
    } catch (error) {
  ...
  const handleReApprove = async (listingId: string) => {
    if (!user) return;
    setActionLoading(listingId);
    try {
      await approveListing(listingId, user.uid);
      ...
      console.error("Error re-approving listing:", error);
    } finally {
      setActionLoading(null);
    }
  };
      ...
      <section className="py-12 md:py-16">
        <div className="wrapper max-w-5xl">
          ...
          <div className="mb-10">
            ...
            <h1 className="text-3xl md:text-4xl font-black">Kiểm Duyệt Tin Đăng Chính</h1>
            <p className="text-zinc-500 mt-1">Duyệt bài tìm phòng, tìm bạn ở ghép</p>
          </div>
          {/* Stats bar */}
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {listingFilters.map(f => (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 text-xs font-bold rounded-lg border-2 transition-all ${listingFilter === f.key
                    ? f.key === "rejected" ? "border-red-600 bg-red-600 text-white" : "border-black bg-zinc-900 text-white"
                    : "border-zinc-300 bg-white text-zinc-600 hover:border-black"
                  }`}
              >
                {f.icon && <f.icon className="w-3.5 h-3.5" />}
                {f.label}
                <span className={`ml-0.5 text-[10px] px-1.5 py-0.5 rounded-full ${listingFilter === f.key ? "bg-white/20" : f.key === "rejected" ? "bg-red-100 text-red-700" : "bg-zinc-100"
                  }`}>
                  {filterCounts[f.key]}
                </span>
              </button>
          ...
          {/* Listing items — Pending filters */}
          {!isLoading && !isRejectedView && filteredPosts.length > 0 && (
          ...
          {/* Rejected — empty */}
          {!isLoading && isRejectedView && rejectedPosts.length === 0 && (
            <div className="text-center py-16 border-2 border-dashed border-zinc-200 rounded-xl bg-white">
              <Ban className="w-12 h-12 text-zinc-300 mx-auto mb-3" />
              <h3 className="text-lg font-bold text-zinc-500 mb-1">Không có bài đăng bị từ chối</h3>
              <p className="text-sm text-zinc-400">Chưa có bài đăng nào bị từ chối</p>
            </div>
          )}
            ...
            <div className="border-2 border-black rounded-xl overflow-hidden bg-white">
              {rejectedPosts.map((post, idx) => {
                ...
                return (
                  <div
                    key={postId}
                    className={`p-5 ${idx !== rejectedPosts.length - 1 ? "border-b-2 border-black" : ""} hover:bg-zinc-50 transition-colors`}
                  >
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                        ...
                        {post.rejectionReason && (
                          <p className="mt-1.5 text-sm text-red-600 bg-red-50 px-3 py-1.5 rounded-lg border border-red-200">
                      ...
                      <div className="flex gap-2 flex-shrink-0">
                        ...
                        <button
                          onClick={() => handleReApprove(postId)}
                          disabled={isActionLoading}
                          className="px-3 py-2 text-sm font-semibold border-2 border-black rounded-lg bg-green-100 hover:bg-green-200 transition-colors flex items-center gap-1 disabled:opacity-50"
                        >
                        ...
                        <button
                          onClick={() => handleDelete(postId)}
                          disabled={isActionLoading}
                          className="px-3 py-2 text-sm font-semibold border-2 border-black rounded-lg bg-red-100 hover:bg-red-200 transition-colors flex items-center gap-1 disabled:opacity-50"
                        >
